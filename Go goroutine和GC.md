...
> ![](https://github.com/Nixum/Java-Note/raw/master/picture/go_分配器结构.png)
>
...
* 使用`go run -gcflags "-m -l"`命令runtime加入参数，-m表示打印逃逸分析信息，-l 表示禁止内联编译
...
go的GC，采用的就是**主体并发增量式垃圾回收**，采用插入写屏障和删除写屏障组合而成的**混合写屏障**；（主体 增量 指的是将STW拆分成多份，与用户程序交替在不同的核中运行，使得可以在垃圾回收开始时，消息能及时准确的通知到所有线程，避免某些线程开启写屏障的动作有所延迟而发生错误）

![](https://github.com/Nixum/Java-Note/raw/master/picture/并发GC模式.png)
...
[[幼麟实现室]golang合辑](https://www.bilibili.com/video/BV1hv411x7we/?p=19&vd_source=a870d050aec1403366ebea0f42b4cdaf)
