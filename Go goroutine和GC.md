...
* 1.2~1.3版本使用**基于协作的抢占式调度器**（通过编译器在函数调用时插入抢占式检查指令，在函数调用时检查当前goroutine是否发起抢占式请求），但gouroutine可能会因为垃圾回收和循环长时间占用资源导致程序暂停。
  ...
  之所以要使用抢占式的，是因为如果是goroutine主动让出CPU资源才能触发调度，可能会导致某个goroutine长时间占用线程，造成其他goroutine饿死；另外，垃圾回收需要暂停整个程序，在STW时，整个程序无法工作。
...
* P：即processor，处理器的抽象，运行在线程上的本地调度器，用来管理和执行goroutine，使得goroutine在一个线程上跑，提供了线程需要的上下文，（局部计算资源，用于在同一线程写多个goroutine的切换），负责调度线程上的LRQ，是实现从N：1到N：M映射的关键。存在的意义在于工作窃取算法。
  ...
  * p的个数取决于**GOMAXPROCS**，默认使用CPU的个数，这些P会绑定到不同内核线程，尽量提升性能，让每个核都有代码在跑。在确定了P的最大数量n后，当程序运行时，创建n个p。
...
* 全局运行队列GRQ（Global Run Queue）：由调度器本身持有，保存所有未分配的goroutine，存于全局遍历sched中。GRQ是一个链表，由head，tail两个指针。
...
* 持有P的M会执行G，完了之后从P的LRQ中继续获取G进行消费

* 持有P的M在执行完G后，发现P的LRQ中没有其他G可执行时，会去消费GRQ里的G，获取时会加锁

* 当P的LRQ和调度器的GRQ都没有可执行的G时，就会从其他P的LRQ的尾部偷取一个G来执行（work stealing工作窃取策略），否则，M进入自旋状态。
  ...
  当M发现自己的LRQ、GRQ都没有G了，则会从其他M中窃取一半的G放到自己的LRQ。

* 当M执行的G发生阻塞时，将挂起当前M和G，从当前P中摘除，唤醒其他M服务这个P，执行其他G。

  阻塞结束后，该M和G会尝试找回原来的P，如果原来的P没空，则获取其他空闲的P，放入其本地运行队列，等待执行；如果没有空闲的P，则G放入全局队列，M继续休眠。（hand off策略）

* 运行时的G会尝试唤醒其他空闲的M和P进行组合，被唤醒的M和P由于刚被唤醒，进入自旋状态，G0发现P的本地队列没有G，则会从全局队列里获取G放入本地队列，获取数量`n = min(len(GQ)/GOMAXPROCS + 1, len(GQ/2))`，一次不会获取太多，保证其他M在GRQ中有G可拿。
...
* 执行栈：每个goroutine包含自己的执行栈，这些执行栈上包含变量以及指向分配的堆内存的指针
...
尚未被垃圾回收器扫描过，或者尽管已经扫描过了，但仍然需要重新扫描。结束时需要STW来重新扫描栈，标记栈上引用的白色对象的存活。
...
已经被垃圾回收器扫描过，不会再次对其进行扫描。回收精度低，GC开始时STW扫描栈来记录初始快照，这个过程会保护开始时刻的所有存活对象。
...
混合屏障结合了删除写屏障和插入写屏障的优点，只需要在开始时并发扫描各个goroutine的栈，使其变黑并一直保持，这个过程不需要STW，而标记结束后，因为栈在扫描后始终是黑色的，也无需再进行re-scan操作了，减少了STW的时间。
