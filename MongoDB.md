...
* 文档按照写入顺序存储在磁盘上，由于文档可能会更新导致长度变长，如果原有存储空间不够，文档就需要进行迁移，导致集合中所有索引都要同步修改文档新的存储位置，影响性能，因此会有两种策略来避免这种情况发生：
...
>
> 　　对于第一种策略，由于每个文档大小不一，经过填充后的空间大小也不一样，如果集合上的更新操作很多，那么因为记录移动而导致的空闲空间会因为大小不一而难以重用。而第二种策略就不一样了，它分配的空间大小都是2的N次方，因此更容易维护和利用。
>
...
LSM树一般由两个或以上的存储结构组成，比如有C0 tree，一般是用于查找的数据结构，比如红黑树、map、跳表之类，另一个结构常驻在硬盘，为C1 tree，比如B+树，C1所有节点都是100%满的，节点的大小为磁盘块的大小；
