...
func main() {
    // 10s内只允许通过10次
	f1 := limitstrategy.NewFixWinLimit(10, 10)
	for i := 0; i < 20; i++ {
		go func(index int) {
			if f1.IsPass() {
...
type slidingWinLimiter struct {
	lock        *sync.Mutex
	lastReqTime int64 // 上次请求时间，单位：秒

	currentWinIndex  int64  // 当前窗口索引
	countInterval    int64  // 计数时间间隔, 单位：秒
	currentWinCount  []int64 // 当前窗口计数值
	maxWinLimitCount int64 // 窗口最大计数值
	winCount         int64 // 窗口数量
}

func NewSlidingWinLimiter(countInterval int64, maxWinLimitCount int64, winCount int64) *slidingWinLimiter {
	return &slidingWinLimiter{
		lock:             new(sync.Mutex),
		countInterval:    countInterval,
		currentWinCount:  make([]int64, maxWinLimitCount),
		maxWinLimitCount: maxWinLimitCount,
		winCount:         winCount,
	}
}
...
func (s *slidingWinLimiter) IsPass() bool {
	s.lock.Lock()
	defer s.lock.Unlock()
	eachWinInterval := s.countInterval / s.winCount
	now := time.Now().Unix()
	if now - s.lastReqTime > eachWinInterval {
		s.currentWinCount[s.currentWinIndex] = 0
		s.currentWinIndex = (s.currentWinIndex + 1) % s.winCount
		s.lastReqTime = now
	}
	if s.currentWinCount[s.currentWinIndex] < s.maxWinLimitCount {
		s.currentWinCount[s.currentWinIndex]++
		return true
	}
	return false
}
...
func main() {
	f1 := limitstrategy.NewSlidingWinLimiter(1, 10, 1)
	for i := 0; i < 20; i++ {
		go func(index int) {
			if f1.IsPass() {
				fmt.Println(fmt.Sprintf("pass: %d", index))
