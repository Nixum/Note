...
比如header里保存的加密方式是HMACSHA256，签名 signature = HMACSHA256(base64URL(header) + "." + base64URL(payload) + "." + 保存在后端的密钥)，最后组合成`base64URL(header) + "." + base64URL(payload) + "." + signature`，后端收到该JWT后验证该签名是否正确，来判断JWT里的用户信息是否可靠。
...
| 被压测服务的gc                        | fgc，ygc不要太频繁，一般来说fgc 一小时要小于3~4次；ygc一分钟要小于3~4次为佳。 |
...
​	2、收集线程堆栈运行信息：`jstack [pid] > [文件名，stack.log]`
