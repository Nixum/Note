...
比如header里保存的加密方式是HMACSHA256，签名 signature = HMACSHA256(base64URL(header) + "." + base64URL(payload) + "." + 保存在后端的密钥)，最后组合成`base64URL(header) + "." + base64(payload) + "." + signature`，后端收到该JWT后验证该签名是否正确，来判断JWT里的用户信息是否可靠。
