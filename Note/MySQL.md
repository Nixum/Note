...
* 临时表，临时表不同于内存表，他是写磁盘的，创建语句是 ```create temporary table ···```，用法跟普通表类似，但仅在当前线程可见，show tables不显示临时表，当当前线程处理完成后，临时表会被清空，但会保留表结构，允许在不同线程间重名，如果bin log格式是statement或mix，操作临时表的记录也会记录在bin log，备库也会跟着操作。临时表常用于不带索引的join、分库分表时的联合查询

* 内存临时表，常用于union查询(对两个查询结果求并集并去重，union all则不去重)；group by查询，group by查询需要构造一个有唯一索引的临时表，算出结果后按 group by 后的字段排序输出，所以在group by 后面的字段尽量要带索引，没有排序需求再加上 ```order by null```。当查询的数据量比较大时，默认会先使用内存临时表，然后发现太大才转成磁盘表，因此如果数据量太大，可以通过在 ```select SQL_BIG_RESULT 字段 from 表 group by xxx```强制使用磁盘表，一般情况下还是使用内存表快些，通过调大tmp_table_size来加大内存临时表的大小
