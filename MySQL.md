...
1. 查询过程中扫描到的行才会加锁，**锁的基本单位是next-key lock（左开右闭），间隙锁是左开右开**；
...
3. 唯一索引范围查询：会对每个扫描到的索引加next-key锁

   * 大于，对扫描到的索引加next-key锁（左开右闭）

     比如有记录id：1，5，10，15，20，查询id>20，加锁区间是 (15, 20], (20, +∞]

   * 大于等于（此时不管条件里有没有小于），等值的记录加行锁，大于该值的记录会进行扫描，扫描到第一个不满足条件的值（也是对应存在的行，不管小于），加间隙锁（左开右闭）

     比如有记录 id：1，5，10，15，20，查询 id>=10 and id <11，加锁区间是 [10], (10, 15]

     查询 id >= 15，加锁区间是 [15], (15, 20], (20,  +∞)

   * 小于或小于等于，且记录不存在，扫描到终止范围查询的记录时，该记录的索引的next-key锁会退化成间隙锁，其他扫描到的记录，还是加next-key锁
...
6. **锁是加在索引上的，先对where条件上的用到的索引加锁，再对该行上存在的索引依次加锁**。比如有表A，索引为id、a、b，当update时的条件是where a=xx时，会对索引a、id、b的顺序加锁；**间隙锁只会加在where条件中的索引上**，对于该索引定位到的行加的是行锁，但对于主键索引加锁的时候，只有满足查询条件的记录才会对它的主键索引加锁；

7. select加的锁，如果查询的列刚好是满足覆盖索引，且覆盖索引不包括其他索引，则只会锁where条件上的索引；
